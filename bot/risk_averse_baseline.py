...
def agent(observation, configuration):
    ...
    for unit in player.units:
        if unit.can_act():
            ...
            if unit.is_worker() and unit.can_build(game_state.map) and (
            ...
            if (
                    steps_to_city + 3 > steps_until_night or unit.get_cargo_space_left() == 0) and closest_city_tile is not None:
